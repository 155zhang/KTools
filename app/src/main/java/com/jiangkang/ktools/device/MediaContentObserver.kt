...
class MediaContentObserver(private val mContext: Context, handler: Handler, private val mContentUri: Uri) : ContentObserver(handler) {
    ...
    companion object {
        ...
        private val FILENAME_FILTER = arrayOf("screenshot", "screen_shot", "screen-shot", "screen shot", "screencapture", "screen_capture", "screen-capture", "screen capture", "screencap", "screen_cap", "screen-cap", "screen cap")

        private val MEDIA_PROJECTIONS = arrayOf(MediaStore.Images.ImageColumns.DATA, MediaStore.Images.ImageColumns.DATE_TAKEN)

        private val MEDIA_SORT_ORDER = MediaStore.Images.ImageColumns.DATE_ADDED + " desc limit 1"
